...
MAPPING = {
    ...
    'lecture_doc': ('Materialele didactice puse la dispozitie sunt suficiente pentru intelegerea cursului', True),
    ...
    for cat in data:
        try:
            ...
            c.execute('''INSERT OR REPLACE INTO categories
                        ...
                        VALUES (?, ?, ?, ?, ?, ?)''',
                        (cat['id'], cat['name'], parent_id, cat['path'], cat['depth'],
                        cat.get('sortorder')))
            count += 1
        except Exception as e:
            print(f"Eroare: {e}.")
    ...
    for course in courses_data:
        ...
        try:
            c.execute('''INSERT OR REPLACE INTO courses
                         ...
                         VALUES (?, ?, ?, ?, ?, ?)''',
                         (course_id, course['shortname'], course['fullname'],
                          course['startdate'], course['enddate'], cat_id))
            count += 1
        except Exception as e:
...
def migrate_feedbacks(conn):
    ...
    exclude = ['categories.json', 'courses.json', 'courses4categories.json', 'feedbacks.json']
    ...
    for filepath in feedback_files:
        ...
        for att in attempts:
            row_data = {
                    'attempt_id': att['id'], #aici trebuie adaugate restul rasp.
                    'instance_id': instance_id,
                    'prof_name': None, 'assist_name': None,
                    'eval_overall': None, 'expected_grade': None,
                    'load': None, 'equipment': None,
                    'part_percent': None, 'prof_know': None,
                    'prof_teach': None,
                    'prof_behave': None, 'lecture_doc': None,
                    'assist_know': None, 'assist_teach': None,
                    'assist_interact': None, 'assist_behave': None, 
                    'lab_doc': None, 'assign_time': None,
                    'assign_diff': None, 'assign_useful': None, 
                    'positive_text': None, 'negative_text': None,
                    'difficulty_text': None, 'other_text': None
                    }
            know_count = 0
            behave_count = 0
            ...
            for r in att['responses']:
                ...
                if "Teacher" in q_text: row_data['prof_name'] = print_val
                elif "Laboratory" in q_text: row_data['assist_name'] = print_val

                for db_col, (keyword, do_invert) in MAPPING.items():
                    if keyword in q_text:
                        if db_col not in ['prof_know', 'assist_know', 'prof_behave',
                                        'assist_behave']:
                            row_data[db_col] = parse_val(raw, do_invert)

                if 'Cadrul didactic stăpânește bine' in q_text:
                    val = parse_val(raw, True)
                    if know_count == 0: row_data['prof_know'] = val
                    else: row_data['assist_know'] = val
                    know_count += 1

                if 'Comportamentul cadrului didactic' in q_text:
                    val = parse_val(raw, True)
                    if behave_count == 0: row_data['prof_behave'] = val
                    else: row_data['assist_behave'] = val
                    behave_count += 1
            ...
            c.execute(f"INSERT OR REPLACE INTO feedback_responses ({cols}) VALUES ({placehld})", values)
...
def migrate_instances(conn):
    ...
    for item in data:
        ...
        c.execute('''INSERT OR REPLACE INTO feedback_instances
